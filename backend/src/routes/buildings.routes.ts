...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const buildings = await BuildingsService.list(clientId);
    res.json(buildings);
  } catch (err) {
...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    const clientId = resolveClientId(req.user!); // undefined for SUPER_ADMIN
    const building = await BuildingsService.create(parsed.data.name, parsed.data.address, clientId);
...
router.delete('/:id', async (req: Request<{ id: string }>, res: Response) => {
  try {
    const clientId = resolveClientId(req.user!);
    await BuildingsService.delete(req.params.id, clientId);
    res.status(204).send();
  } catch (err) {
